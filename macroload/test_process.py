...
def test_create_output_row():
    row = extract.ValidatedResultTest(extract.DatedSubjectSpecificTests([
    ...
    visit_details = core.SubjectVisitDetails(study_id = STUDY_ID, visit_no=VISIT_CODE, visit_date = TEST_DATE)
    obs_prow = process.create_processed_row(row, visit_details, core.TestInfo())
    ...
    exp_data.update({
        ...
        "Visit Date": TEST_DATE.strftime(config.DATE_OUTPUT_FORMAT),
