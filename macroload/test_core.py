...
def test_extract_subject_visit_tests_works_with_no_errors():
    ...
    visit = core.SubjectVisitDetails(study_id="12345", visit_date=dt.date(2018, 3, 2), visit_no="vst1")
...
def test_test_map_returns_expected_test_code():
    test_map = core.TestMap([
        {"test_code": "EOSAB", "var_code": "eosinophil"},
    ...
    assert test_map.get("eosinophil") == ["EOSAB","EOS"]
    assert test_map.var_for_test("EOSAB") == "eosinophil"
    assert test_map.var_for_test("EOS") == "eosinophil"
    assert test_map.var_for_test("PLT") == "platelet"

def test_test_map_with_none_test_code_fails():
    with pytest.raises(ValueError):
        core.TestMap([
            {"test_code": "EOSAB", "var_code": "eosinophil"},
            {"test_code": None, "var_code": "eosinophil"},
            {"test_code": "NEU", "var_code": "neutrophil"},
            {"test_code": "PLT", "var_code": "platelet"}
        ])
